...
/**
 * Created by Gary Kasameyer on 11/3/2017.
...
public class Exercise_31 {
    ...
    public static int sumOfDoubleEvenPlace(long number){
        ...
        for(long i = 10; i <= number; i *= 100){
            long tempLong = number / i % 10;
            tempLong *= 2;
            if((int)tempLong <= 9)
                sumOfDoubleEvenPlaces += (int)tempLong;
            else if((int)tempLong > 9)
                sumOfDoubleEvenPlaces += getDigit((int)tempLong);
        }
     ...
     * number of digits in number is less than k, return number. */
    public static long getPrefix(long number, int k){
        String whole = String.valueOf(number);
        String pre = String.valueOf(k);

        if(getSize(number) == getSize(k))
            return k;
        else if(whole.substring(0, getSize(k)).equals(pre.substring(0, getSize(k))))
            return Long.parseLong(whole.substring(0, getSize(k)));
        else
            return 0;
